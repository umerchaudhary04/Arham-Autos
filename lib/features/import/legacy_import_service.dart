...
class LegacyImportService {
  ...
  Map<String, dynamic> validateAndMap(Map<String, dynamic> rawData) {
    ...
    if (rawData.containsKey('Items')) {
      for (var item in rawData['Items']) {
        if (item['PartName'] == null || item['PartName'].toString().trim().isEmpty) {
          skippedRecords.add({'table': 'Items', 'reason': 'Missing PartName', 'data': item});
          continue;
        }
        validParts.add({
  ...
  Future<void> commitImport(List<Map<String, dynamic>> partsWithStock, List<Map<String, dynamic>> skippedRecords) async {
    ...
    await db.transaction(() async {
      ...
      // Write MigrationLog
      await db.into(db.migrationLog).insert(MigrationLogCompanion.insert(

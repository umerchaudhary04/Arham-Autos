...
class ReportsDao extends DatabaseAccessor<AppDatabase> with _$ReportsDaoMixin {
  ...
  Future<double> getTodayProfit() async {
    ...
    final invoices = await (select(salesInvoices)
      ..where((i) => i.createdAt.isBiggerOrEqualValue(startOfDay)))
      .get();
    ...
    for (var inv in invoices) {
      final items = await (select(invoiceItems)..where((item) => item.invoiceId.equals(inv.invoiceId))).get();
      double cost = items.fold(0.0, (sum, item) => sum + (item.unitCogs * item.quantity));

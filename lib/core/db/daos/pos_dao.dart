...
@DriftAccessor(tables: [SalesInvoices, InvoiceItems, FifoInventoryBatches, AccountsLedger])
class PosDao extends DatabaseAccessor<AppDatabase> with _$PosDaoMixin {
  ...
  Future<String> processSale({
    ...
    return transaction(() async {
      ...
      // 1. Create Invoice
      await into(salesInvoices).insert(SalesInvoicesCompanion.insert(
        invoiceId: invoiceId,
      ...
      for (final item in items) {
        ...
        final batches = await (select(fifoInventoryBatches)
              ..where((b) => b.partId.equals(item.partId) & b.remainingQuantity.isBiggerThanValue(0))
        ...
        for (final batch in batches) {
          ...
          final deductQty = qtyRemainingToDeduct <= batch.remainingQuantity ? qtyRemainingToDeduct : batch.remainingQuantity;
          qtyRemainingToDeduct -= deductQty;

          // Deduct from batch
          await update(fifoInventoryBatches).replace(
            batch.copyWith(remainingQuantity: batch.remainingQuantity - deductQty)
          );
        ...
        if (qtyRemainingToDeduct > 0) {
          throw Exception("Strict stock over-sell block: Not enough stock for part ${item.partId}. Missing: $qtyRemainingToDeduct");
        }
      }

      // 3. Ledger Update (if on credit)
      if (customerId != null) {
        final account = await (select(accountsLedger)..where((a) => a.accountId.equals(customerId))).getSingle();
        ...
        await update(accountsLedger).replace(account.copyWith(currentBalance: newBalance));
      }
...
class CartItem {
  ...
  CartItem({required this.partId, required this.quantity, required this.unitPrice});
}
